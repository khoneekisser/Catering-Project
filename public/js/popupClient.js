...
//     const showClientsLink = document.getElementById('showCustomers');
...
//                         const imageUrl = details.Logo ? `/storage/${details.Logo}` : 'storage/picture/1FO47Vd4msCwkshOY5QlJfjc0MZBMR9bZUgulcMJ.jpg';
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const showCustomersLink = document.getElementById('showCustomers');
    const popupWrapper = document.getElementById('popupWrapper');
    const closePopupButton = document.getElementById('closePopup');
    const popup = popupWrapper.querySelector('.popup');
    const searchInputProfileSearch = document.getElementById('searchInputProfileSearch');
    ...
    let filteredData = [];
    let isShowingOwners = true;

    // Event listener for "show owners" link
    showClientsLink.addEventListener('click', function(event) {
        event.preventDefault();
        isShowingOwners = true;
        searchInputProfileSearch.value = ''; // Clear search input
        // Show the popup window
        popupWrapper.style.display = 'block';
    ...
    // Event listener for "show customers" link
    showCustomersLink.addEventListener('click', function(event) {
        event.preventDefault();
        isShowingOwners = false;
        searchInputProfileSearch.value = ''; // Clear search input
        // Show the popup window
        popupWrapper.style.display = 'block';
    ...
    searchInputProfileSearch.addEventListener('input', function() {
    ...
    function filterData() {
        const searchTerm = searchInputProfileSearch.value.toLowerCase();
        if (isShowingOwners) {
            filteredData = owners.filter(client => client.user_name.toLowerCase().includes(searchTerm));
        } else {
            filteredData = customers.filter(client => client.user_name.toLowerCase().includes(searchTerm));
        }
        populateClientsTable();
    }
    ...
    function populateClientsTable() {
        ...
        pageData.forEach(function(client) {
            ...
            row.innerHTML = `
                ...
                <td class="is-active">${client.Is_Active ? 'Yes' : 'No'}</td>
                <td><button class="toggle-active" data-id="${client.id}" data-active="${client.Is_Active}">${client.Is_Active ? 'Disable' : 'Enable'}</button></td>
            `;
            tbody.appendChild(row);
        });
        ...
        document.querySelectorAll('.view-client').forEach(function(link) {
            link.addEventListener('click', function(event) {
                ...
                if (detailsRow) {
                    ...
                    newRow.id = `clientDetails-${clientId}`;
                    ...
                    if (isShowingOwners) {
                        ...
                        // Loop through customerDetails[clientId] and generate table rows
                        customerDetails[clientId].forEach(function(details) {
                            const imageUrl = details.Logo ? `/storage/${details.Logo}` : 'storage/picture/1FO47Vd4msCwkshOY5QlJfjc0MZBMR9bZUgulcMJ.jpg';
...
>
                    ...
                    this.closest('tr').insertAdjacentElement('afterend', newRow);
                    openDetailsCount++;

                    if (openDetailsCount === 1) {
                        popup.style.width = '60%'; // Expand width when details are opened
                    }
                }
            });
        });
